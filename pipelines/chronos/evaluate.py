...
def run(labeler, head_factory=None, seeds=(0, 1, 2), train_m=3, test_m=1,
    ...
    fold-seed plus an aggregate stats block + per-ticker breakdown for
    REAL. Returns the per-(fold,seed) records."""
    ...
    for fold, tr, te in walk_forward_folds(labeler.calendar(), train_m,
        ...
        for seed in seeds:
            p_real = (head_factory(labeler.n_classes)
            ...
            # pool for aggregate (with per-trade tickers)
