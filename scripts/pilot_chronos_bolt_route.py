#!/usr/bin/env python3
"""Run a bounded, non-OOS Chronos-Bolt native-quantile pilot.

The pilot uses one externally hash-bound tournament cache, exact-cadence hard
boundaries, identical fixed validation exposure for vanilla and adapted states,
and one elimination seed.  It cannot authorize promotion or full training because
causal-feature and residual-over-causal downstream gates are not part of this run.
"""
...
RAW_SCHEMA = "ffm_chronos_bolt_bounded_pilot_raw_v1"
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, len(schedule), batch_size):
            ...
            total += float(route.native_loss(model, parent, device=device).detach().cpu())
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if smoke["route_key"] != route.ROUTE_KEY or smoke["smoke_admitted"] is not True:
        raise ValueError("Chronos-Bolt pilot requires passing smoke for the exact route")
    if Path(smoke["executor"]["path"]).resolve() != Path(route.__file__).resolve():
        raise ValueError("Chronos-Bolt pilot smoke executor differs from current route code")
    ...
    if args.eval_every < 1 or config.total_steps % args.eval_every:
        raise ValueError("eval_every must be positive and divide total pilot steps")
    ...
    pipeline, model, model_identity = route.load_pipeline(
    ...
    for step in range(config.total_steps):
        ...
        if completed % args.eval_every == 0:
            ...
            if not args.quiet:
                print(
                    f"[chronos-bolt pilot] step={completed} "
    ...
    if best_step < 1:
        raise RuntimeError("Chronos-Bolt pilot produced no validation checkpoint")

    # Preserve the exact final optimizer trajectory for resume.
    ...
    training_state_path = output / "chronos_bolt_pilot.train.pt"
    training_state_identity = route.save_training_state(training_state_path, state)

    # Deployment uses the best validation state, never the final optimizer state implicitly.
    ...
    deployment_path = output / "chronos_bolt_pilot.forecast.pt"
    ...
    if not np.isclose(adapted_validation, best_validation, rtol=0.0, atol=1e-8):
        raise RuntimeError("best deployment validation loss is not reproducible")
