#!/usr/bin/env python3
"""Run the bounded, non-OOS Kronos Mini tokenizer-reconstruction pilot.

This command trains only the canonical tokenizer route.  The predictor remains
untouched.  A passing result supplies a tokenizer parent candidate for the
hierarchical predictor route but cannot authorize promotion, full training,
OOS access, deployment, or trading.
"""
...
ARM_KEY = "kronos_mini"
ROUTE_KEY = route.route_key(ARM_KEY)
RAW_SCHEMA = "ffm_kronos_mini_tokenizer_bounded_pilot_raw_v1"
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, len(schedule), batch_size):
            ...
            total += float(
                route.native_loss(tokenizer, parent, device=device).detach().cpu()
            )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if smoke["route_key"] != ROUTE_KEY or smoke["smoke_admitted"] is not True:
        raise ValueError("Kronos Mini pilot requires passing smoke for the tokenizer route")
    if Path(smoke["executor"]["path"]).resolve() != Path(route.__file__).resolve():
        raise ValueError("Kronos Mini pilot smoke executor differs from current route code")
    smoke_model = Path(smoke["artifacts"]["model_snapshot"]["path"]).resolve()
    smoke_tokenizer = Path(smoke["artifacts"]["tokenizer_snapshot"]["path"]).resolve()
    smoke_source = Path(smoke["artifacts"]["source_runtime"]["path"]).resolve()
    if smoke_model != Path(args.model_snapshot).resolve():
        raise ValueError("Kronos Mini predictor snapshot differs from smoke")
    if smoke_tokenizer != Path(args.tokenizer_snapshot).resolve():
        raise ValueError("Kronos Mini tokenizer snapshot differs from smoke")
    if smoke_source != Path(args.source_runtime).resolve():
        raise ValueError("Kronos Mini source runtime differs from smoke")
    ...
    if not 128 <= config.total_steps <= 2048:
        raise ValueError("bounded tokenizer pilot steps must lie in [128,2048]")
    ...
    tokenizer, model_identity = route.load_tokenizer(
        ARM_KEY,
    ...
    route.seed_everything(config.seed)
    optimizer = route.make_optimizer(tokenizer, config)
    scheduler = route.make_scheduler(optimizer, config)
    best_validation = float("inf")
    best_step = 0
    best_model = route.model_state_cpu(tokenizer)
    ...
    for step in range(config.total_steps):
        ...
        if completed % args.eval_every == 0:
            ...
            if not args.quiet:
                print(
                    f"[kronos-mini tokenizer pilot] step={completed} "
    ...
    if best_step < 1:
        raise RuntimeError("Kronos Mini tokenizer pilot produced no checkpoint")
    ...
    training_state_path = output / "kronos_mini_tokenizer_pilot.train.pt"
    training_state_identity = route.save_training_state(training_state_path, state)

    tokenizer.load_state_dict(best_model, strict=True)
    tokenizer.eval()
    export = route.build_export_bundle(
        arm_key=ARM_KEY,
        tokenizer=tokenizer,
        model_identity=model_identity,
    )
    deployment_path = output / "kronos_mini_tokenizer_pilot.bundle.pt"
    ...
    if not np.isclose(adapted_validation, best_validation, rtol=0.0, atol=1e-8):
        raise RuntimeError("Kronos Mini best tokenizer loss is not reproducible")
    ...
    raw = {
        ...
        "predictor_route_admitted": False,
