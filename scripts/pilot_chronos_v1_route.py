#!/usr/bin/env python3
"""Run a bounded, non-OOS Chronos V1 native-token pilot."""
...
RAW_SCHEMA = "ffm_chronos_v1_bounded_pilot_raw_v1"
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, len(schedule), batch_size):
            ...
            total += float(
                route.native_loss(
                    pipeline, model, parent, device=device,
                ).detach().cpu()
            )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if smoke["route_key"] != route.ROUTE_KEY or smoke["smoke_admitted"] is not True:
        raise ValueError("Chronos V1 pilot requires passing smoke for the exact route")
    if Path(smoke["executor"]["path"]).resolve() != Path(route.__file__).resolve():
        raise ValueError("Chronos V1 pilot smoke executor differs from current route code")
    ...
    vanilla_validation = _evaluate(
        pipeline, model, big, validation_schedule,
        batch_size=config.batch_size, device=args.device,
    )
    ...
    for step in range(config.total_steps):
        ...
        row = route.optimizer_step(
            pipeline, model, optimizer, scheduler, parent,
            device=args.device,
        ...
        if completed % args.eval_every == 0:
            ...
            if not args.quiet:
                print(
                    f"[chronos-v1 pilot] step={completed} "
    ...
    if best_step < 1:
        raise RuntimeError("Chronos V1 pilot produced no validation checkpoint")
    ...
    training_state_path = output / "chronos_v1_pilot.train.pt"
    ...
    deployment_path = output / "chronos_v1_pilot.forecast.pt"
    ...
    if not np.isclose(adapted_validation, best_validation, rtol=0.0, atol=1e-8):
        raise RuntimeError("Chronos V1 best deployment loss is not reproducible")
    ...
    evidence = build_route_pilot_evidence(
        ...
        executor_entrypoint="native_loss/forecast_samples",
