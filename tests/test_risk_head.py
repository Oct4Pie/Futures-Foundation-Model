...
def _key(realized):                                      # (name,i,d,peak, R@2,R@3,R@4,R@6)
    return ('ES@3min', 0, 1, 0.0, *realized)


# ---------------------------------------------------------------- labels from keys
def test_reach_labels_from_keys():
    keys = [_key([1.97, 2.97, -0.5, -0.5]),              # reached 3R (not 4R/6R)
            _key([-1.03, -1.03, -1.03, -1.03]),          # stopped immediately
            _key([1.97, 2.97, 3.97, 5.97])]              # ran to 6R
    lab = reach_labels(keys)
    assert lab.shape == (3, 4)
    assert lab[0].tolist() == [1, 1, 0, 0]
    assert lab[1].tolist() == [0, 0, 0, 0]
    assert lab[2].tolist() == [1, 1, 1, 1]


# ---------------------------------------------------------------- survival curve is valid
def test_monotone_survival_enforced_and_clipped():
    s = monotone_survival([[0.3, 0.5, 0.2, 0.1], [1.2, 0.9, 0.9, -0.1]])
    assert np.all(np.diff(s, axis=1) <= 1e-12)           # non-increasing
    assert s.min() >= 0 and s.max() <= 1                 # clipped to [0,1]
    assert np.allclose(s[0], [0.3, 0.3, 0.2, 0.1])       # 0.5 pulled down to 0.3


def test_survival_to_stats_always_reaches():
    st = survival_to_stats([[1, 1, 1, 1]])               # certain to reach 6R
    assert np.isclose(st['exp_reach'][0], TARGETS[-1])   # E[peak] == 6
    assert st['tp'][0] == TARGETS[-1] and st['p_bigwin'][0] == 1.0


def test_survival_to_stats_reaches_2R_only():
    st = survival_to_stats([[1, 0, 0, 0]])
...
def test_survival_to_stats_dynamic_tp_threshold():
    # q_tp=0.33: reach probs [0.5,0.4,0.3,0.2] -> only 2R(0.5) & 3R(0.4) clear 0.33 -> TP=3R
    st = survival_to_stats([[0.5, 0.4, 0.3, 0.2]], q_tp=0.33)
    assert st['tp'][0] == 3.0
...
def _toy(n, D=8, seed=0):
    ...
    w = rng.normal(size=D)
    s = X @ w
    offs = [0.0, 0.8, 1.6, 2.8]                           # higher target = harder
    keys = []
    for r in range(n):
...
def test_degenerate_threshold_is_constant_head():
    ...
    keys = [_key([1.0, -1.0, -1.0, -1.0]) for _ in range(300)]   # 3R/4R/6R never hit
    h = RiskHead(head='logistic', calibrate=True).fit(X, keys, X, keys)
    surv = h.predict_survival(X)
    assert surv.shape == (300, 4)
